...
const jobURL = new URL(process.env.NODE_ENV === 'development' ? config.POLLING_URL_DEV : config.POLLING_URL)
const completeURL = (process.env.NODE_ENV === 'development' ? config.COMPLETE_URL_DEV : config.COMPLETE_URL)
...
const poll = async () => {
    ...
    console.log(`Stage 1 found ${json}`);
